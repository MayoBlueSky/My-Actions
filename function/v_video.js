...
exports.main = () => new Promise(
    (resovle, reject) => refCookie()
        .then(params=>Promise.all([ txVideoSignIn(params), txVideoCheckin(params)])
            .then(e=>resovle(notify.sendNotify("腾讯视频签到通知",...e)))
            .catch(e=>reject(notify.sendNotify("腾讯视频签到通知",...e)))
        ).catch(e=>{
            //如果有错误自行取消下面这行注释
            //console.log(e)
            console.log('腾讯视频签到通知-Cookie已失效')
            notify.sendNotify("腾讯视频签到通知", "获取Cookie失败，大概Cookie失效")
        })
)
console.log('腾讯视频会员开始签到')
exports.main()
