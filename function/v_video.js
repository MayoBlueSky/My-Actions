...
function txVideoDownTask1(headers) {
    $.get({
        url: `https://vip.video.qq.com/fcgi-bin/comm_cgi?name=spp_MissionFaHuo&cmd=4&task_id=7&_=1582364733058&callback=Zepto1582364712694`, headers
    }, function(error, response, data) {
        if (error) {
            $.log(error);
...
function txVideoDownTask2(headers) {
    $.get({
        url: `https://vip.video.qq.com/fcgi-bin/comm_cgi?name=spp_MissionFaHuo&cmd=4&task_id=6&_=1582366326994&callback=Zepto1582366310545`, headers
    }, function(error, response, data) {
        if (error) {
            $.log(error);
...
//弹幕任务签到请求
//https://vip.video.qq.com/fcgi-bin/comm_cgi?name=spp_MissionFaHuo&cmd=4&task_id=3
function txVideoDownTask3(headers) {
    $.get({
        url: `https://vip.video.qq.com/fcgi-bin/comm_cgi?name=spp_MissionFaHuo&cmd=4&task_id=3&_=1582368319252&callback=Zepto1582368297765`, headers
    }, function(error, response, data) {
        if (error) {
            $.log(error);
...
function txVideoDownTask4(headers) {
    $.get({
        url: `https://vip.video.qq.com/fcgi-bin/comm_cgi?name=spp_MissionFaHuo&cmd=4&task_id=1&_=1582997048625&callback=Zepto1582997031843`, headers
    }, function(error, response, data) {
        if (error) {
            $.log(error);
...
//任务领取
exports.misson = () => new Promise(
    (resovle, reject) => refCookie()
        .then(params=>Promise.all([ txVideoDownTask1(params), txVideoDownTask2(params), txVideoDownTask3(params), txVideoDownTask4(params)])
            .then(e=>resovle())
            .catch(e=>reject())
        ).catch(e=>{
            //如果有错误自行取消下面这行注释
